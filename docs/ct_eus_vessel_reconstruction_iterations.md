...
- v13 从 v12b 的两个未解决问题继续推进：
  - 肝内主血管与主干断裂/切断。
  - 肝尖浅表下约 1cm 的片状/破洞网状组织未清理。
- v13 保留 v12b/v10 的 SMV/portal bridge 稳定参数，不再牺牲肠系膜上静脉重建。
- v13 新增肝内主干连通性审计与证据约束重连，并让肝尖浅表下清理在显式 `protection_source: protected_trunk` 时保护重连后的主干。

### 真实输出目录

- Windows:
  - `C:\Users\zhangyutang\Desktop\CT-EUS血管重建结果\无标签\CT-0021-v13-trunk-connect-apex-subsurface`
- WSL:
...
- `voxel_counts`: arterial `89522`，portal `18963`，venous `349099`，fused `592462`
- SMV/portal bridge:
  - `smv_portal_bridge_repair_voxels: 1854`
  - `smv_portal_bridge_repair_pairs: 1`
  - `smv_portal_bridge_repair_evidence_voxels: 1113`
...
- 肝尖浅表下清理:
  - `apex_subsurface_cleanup_candidate_voxels: 25`
  - `apex_subsurface_cleanup_voxels: 0`
  - `apex_subsurface_cleanup_components: 0`
  - `apex_subsurface_cleanup_protected_voxels: 213215`
- 保护与边界:
  - `protected_trunk_voxels: 136616`
  - `outside_body_voxels: 0`
  - `voxels_removed_by_table_gate: 0`

### v13 自动指标解读

- SMV/portal bridge 指标与 v12b 保持一致，说明 V13 没有把肠系膜上静脉桥接能力打回退。
- 肝内主干连通性审计在本次真实病例上显示 before/after 均已连通，因此证据约束重连没有触发；这说明自动审计没有发现可修复断链，但不能替代 Slicer 视觉复核。
- 肝尖浅表下清理本轮没有实际删除体素：候选只有 `25` voxels，低于当前组件体积门槛，且 protected trunk 范围较大。用户复核时仍需重点查看肝尖浅表下约 1cm 位置的片状/破洞网状组织是否仍存在。
